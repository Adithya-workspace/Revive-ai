...
- [x] **Phase 14** — Frontend dashboard: design system, app shell, Dashboard, Revenue at Risk (filterable/paginated table), and Case Detail (full case story + human-in-the-loop approve/reject for NEEDS_HUMAN actions) all complete and verified against live backend data. Remaining screens (Recovery Cases, Customers, Actions, Escalations, Analytics, Audit Trail, Policies, Settings) still to build.
